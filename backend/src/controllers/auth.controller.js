...
exports.login = async (req, res, next) => {
  try {
    ...
    if (!valid) {
      ...
      const shouldLock = attempts >= 12;
      const lockUntil  = shouldLock ? new Date(Date.now() + 8 * 60 * 1000) : null;

      await prisma.user.update({
        where: { id: user.id },
        data: { loginAttempts: shouldLock ? 0 : attempts, lockedUntil: lockUntil },
      });

      return next(new AppError(
        shouldLock
          ? 'Muitas tentativas. Conta bloqueada por 8 minutos.'
  ...
  try {
    ...
    if (!user) {
      return res.status(404).json({
        success: false,
        message: 'Este email não está cadastrado. Verifique o email ou crie uma conta.',
      });
    }
